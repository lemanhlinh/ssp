...
if($lang == 'jp'){
    ?>
    <style>
        .banner-list .title-banner h3{
            font-size: 32px;
        }
        .banner-list .title-banner p{
            font-size: 22px;
        }
    </style>
    <?php
		...
		<?php if($item -> type == 1){?>
			<?php if($item -> image){?>
				<a class="banners-item" href="<?php echo $item -> link;?>" title='<?php echo $item -> name;?>'  id="banner_item_<?php echo $item ->id; ?>">
					...
					<img class="img-responsive"  alt="<?php echo $item -> name; ?>" src="<?php echo URL_ROOT.str_replace('/original/','/original/', $item -> image);?>" width="<?php echo $item -> width;?>" height="<?php echo $item -> height;?>">
					<?php } else { ?>
					<img class="img-responsive" alt="<?php echo $item -> name; ?>" src="<?php echo URL_ROOT.str_replace('/original/','/original/', $item -> image);?>">
					<?php }?>
				</a>
                <div class="title-banner text-center">
                    <?php if($item->is_types){ ?>
                    <?php if($item->name){ ?>
