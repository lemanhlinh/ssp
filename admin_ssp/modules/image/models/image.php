<?php 
	...
	class ImageModelsImage extends FSModels
	{
		...
		function __construct()
		{
			...
			$this->view = 'images';
            $this->type = 'image';
			$this -> table_name = FSTable_ad::_('fs_multimedia');
            $this-> table_name_img = FSTable_ad::_('fs_image_images');
			$this -> arr_img_paths = array(
                                            array('resized',270,152,'cut_image'),
//                                            array('small',182,121,'cut_image'),
//                                            array('large',370,244,'cut_image')
                                        );
			// config for save
			$cyear = date('Y');
			$cmonth = date('m');
		...
		function load_params() {
    		...
    		if ($module_params) { // params from fs_config_modules
    			...
    			if (! $image_resized_width && ! $image_resized_height) {
    				$image_resized_width = 270;
    				$image_resized_height = 180;
    			}
    ...
    //                        echo $image_small_width; die;
    			...
    			// default
    			$this->arr_img_paths = array (array ('large', 370, 244, 'resize_image' ), array ('resized', 270, 180, 'cut_image' ), array ('small', 182, 121, 'resize_image' ) );
    			$this->arr_img_paths_other = array (array ('large', 370, 244, 'resize_image' ), array ('resized', 270, 180, 'cut_image'  ), array ('small', 182, 121, 'resize_image' ) );
    		}
    	}
