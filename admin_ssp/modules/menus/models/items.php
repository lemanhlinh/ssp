<?php 
	class MenusModelsItems extends FSModels
	{
		...
		function __construct()
		{
			...
			$this -> table_name_groups = FSTable_ad::_('fs_menus_groups');
...
//			return $result;
		...
		function getMenuGroups()
		{
			...
			$query = " SELECT group_name, id 
						FROM ".$this -> table_name_groups." WHERE published = 1 ";
...
//				$where .= "  AND = $gid ";
		$query = " SELECT a.*, parent_id as parent_id, b.group_name 
						  ...
						  LEFT JOIN ".$this -> table_name_groups." as b ON a.group_id  = b.id 
...
//			}
			...
			if($image){
				...
				$image = $fsFile -> uploadImage("image", $path ,2000000, '_'.time(),24,24);
				if(!$image)
					return false;
